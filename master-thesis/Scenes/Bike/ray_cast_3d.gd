...
func run_raycast():
	var origin = get_parent().position+get_parent().global_transform.basis.z*1.7
	var space_state = get_world_3d().direct_space_state
	var query = PhysicsRayQueryParameters3D.create(origin, origin-get_parent().global_transform.basis.z*RayCastLength) 
	...
	for y in results: 
		if y["collider"] is Bike_body:
			...
			if n_bikes == 3: 
				dist_to_bike_3th = origin.distance_to(y["position"])
		else:
			print("collided with ", y["collider"].get_parent().name)
		
	return [n_bikes, (sum/max(1, n_bikes)), dist_to_bike_1st ,dist_to_bike_3th,]

func multi_raycast(space_state: PhysicsDirectSpaceState3D, query: PhysicsRayQueryParameters3D, max_iterations: int = 8) -> Array[Dictionary]:
