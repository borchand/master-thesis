...
func _process(delta):
	...
	if self.progress >= max_progress:
		if not is_rl:
			print("Bike: ", self.name, " Finish time: ", total_time, " Remaining_time: ", timer)
...
func control2(_delta):
	...
	behaviorChange(_delta, elevation)
	if behavior == "cruise":
	...
	speed = max(0.5, speed + acceleration * _delta)
